			...
			unsigned long arg)
{
	...
	case MC_ADMIN_IO_AUTHENTICATOR_CHECK: {
		struct mc_admin_authenticator_check info;
		struct service *service = NULL;
	...
	case MC_ADMIN_IO_PID_TO_UID: {
		struct mc_admin_pid_to_uid info;
		struct task_struct *task;
		...
		if (!task) {
			...
			info.uid = __kuid_val(task_uid(task));
			ret = 0;
		}
		rcu_read_unlock();
...
bool mc_is_admin_tgid(pid_t tgid)
{
	...
	mutex_lock(&admin_ctx.services_mutex);
	if (admin_ctx.services[0].role == TEE_ROLE_LISTENER)
		match = admin_ctx.services[0].tgid == tgid;
	else if (admin_ctx.services[1].role == TEE_ROLE_LISTENER)
		match = admin_ctx.services[1].tgid == tgid;
