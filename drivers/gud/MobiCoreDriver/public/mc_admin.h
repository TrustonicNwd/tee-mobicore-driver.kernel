...
struct mc_admin_authenticator_check {
	pid_t		pid;
};

struct mc_admin_pid_to_uid {
	pid_t		pid;
	uid_t		uid;
};
...
#define MC_ADMIN_IO_AUTHENTICATOR_CHECK \
	_IOW(MC_IOC_MAGIC, 6, struct mc_admin_authenticator_check)
#define MC_ADMIN_IO_PID_TO_UID \
	_IOWR(MC_IOC_MAGIC, 7, struct mc_admin_pid_to_uid)
